...
def cadastro_manutencao():
  ...
  manutencao = {}
  cur = conn.cursor()
  id = cur.execute("SELECT COUNT() FROM manutencao")
  id = id.fetchone()[0]
  ...
  if manutencao['nome'] != '' and manutencao['cpf'] != '' and manutencao['detalhe'] != '' and manutencao['valor'] != '' and manutencao['dataEntrada'] != '':
    query = "INSERT INTO manutencao VALUES {0}  ".format(manutencao.values())
...
def menu_principal():
  ...
  while op == True:
    ...
    if(opcao == 1):
      cadastro_manutencao(manutencoes)
    # elif(opcao == 2):
    #   altera_manutencao()
    # elif(opcao == 3):
    #   realiza_manutencao()
